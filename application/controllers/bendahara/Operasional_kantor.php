...
class Operasional_kantor extends CI_Controller {
	...
	public function index()
	{
		...
		$data=array(
			...
			'no_trans'=>$this->M_operasional_all->get_no_trans_kantor(),
			
			);

		$data['operasional_kantor'] = $this->M_operasional_all->get_operasional();

		$this->load->view('bendahara/template/header');
		$this->load->view('bendahara/template/sidebar');
		$this->load->view('bendahara/operasional_kantor', $data);
		$this->load->view('bendahara/template/footer_kantor');
	...
	public function ajax_list()
	{
		...
		$list = $this->M_operasional_all->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $operasioanl) {
			...
			$row[] = '<a class="btn btn-sm btn-info" href="javascript:void(0)" title="Edit" onclick="edit_operasional('."'".$operasioanl->id_operasional."'".')"><i class="glyphicon glyphicon-info-sign"></i> Detail</a>';
